...
async def process_pending_strikes():
    for guild in bot.guilds:
        ...
        # Process pending DM verifications (runs once per loop, not per guild)
        if guild == bot.guilds[0]:
            dms_path = os.path.join(DATA_DIR, "pending_dms.json")
            try:
                with open(dms_path) as f:
                    pending_dms = json.load(f)
                updated = []
                changed = False
                for entry in pending_dms:
                    if not entry.get("sent"):
                        try:
                            dm_user = await bot.fetch_user(int(entry["discord_id"]))
                            await dm_user.send(
                                f"🔐 **AutoMod Dashboard Verification**\n\n"
                                f"Your sign-up code for account **{entry['username']}** is:\n"
                                f"```\n{entry['code']}\n```\n"
                                f"Enter this on the dashboard to activate your account.\n"
                                f"*(This code expires if you request a new one.)*")
                            entry["sent"] = True
                            changed = True
                            print(f"[AutoMod] ✅ Sent verification DM to {entry['discord_id']} for '{entry['username']}'")
                        except Exception as e:
                            print(f"[AutoMod] ⚠️  Could not DM {entry['discord_id']}: {e}")
        ...
        # Process pending moderation actions (kick/ban/unban/roles)
        actions = load_guild(gid, "pending_actions.json", [])
        if actions:
            save_guild(gid, "pending_actions.json", [])
            for entry in actions:
                ...
                try:
                    if act == "kick":
                        ...
                        if member:
                            await member.kick(reason=entry.get("reason", "Dashboard kick"))
                            add_log(gid, "KICK", member, entry.get("reason", "Dashboard kick"))
                            print(f"[AutoMod] Kicked {member} from {guild.name}")

                    elif act == "ban":
                        uid = int(entry["user_id"])
                        member = guild.get_member(uid)
                        reason = entry.get("reason", "Dashboard ban")
                        days   = entry.get("delete_days", 0)
                        if member:
                            await member.ban(reason=reason, delete_message_days=days)
                            add_log(gid, "BAN", member, reason)
                        else:
                            await guild.ban(discord.Object(id=uid), reason=reason, delete_message_days=days)
                            add_log(gid, "BAN", f"User {uid}", reason)
                        print(f"[AutoMod] Banned {uid} from {guild.name}")
                        await save_bans_for_guild(guild)

                    elif act == "unban":
                        uid = int(entry["user_id"])
                        await guild.unban(discord.Object(id=uid), reason=entry.get("reason", "Dashboard unban"))
                        add_log(gid, "UNBAN", f"User {uid}", entry.get("reason", "Dashboard unban"))
                        print(f"[AutoMod] Unbanned {uid} from {guild.name}")
                        await save_bans_for_guild(guild)

                    elif act == "create_role":
                        color_hex = entry.get("color", "99aab5").lstrip("#")
                        color = discord.Color(int(color_hex, 16))
                        role  = await guild.create_role(
                            name=entry["name"], color=color, reason="Dashboard role creation")
                        add_log(gid, "ROLE_CREATE", "AutoMod", f"Created role: {role.name}")
                        print(f"[AutoMod] Created role '{role.name}' in {guild.name}")
                        save_roles_for_guild(guild)

                    elif act == "delete_role":
                        role = guild.get_role(int(entry["role_id"]))
                        if not role:
                            print(f"[AutoMod] ⚠️  delete_role: role {entry['role_id']} not found in {guild.name}")
                        elif role.managed:
                            print(f"[AutoMod] ⚠️  delete_role: '{role.name}' is managed (integration) — cannot delete")
                        else:
                            bot_top = max((r.position for r in guild.me.roles), default=0)
                            if role.position >= bot_top:
                                print(f"[AutoMod] ⚠️  delete_role: '{role.name}' is at/above bot's highest role — cannot delete. Move the bot's role higher in Server Settings.")
                            else:
                                await role.delete(reason="Dashboard role deletion")
                                add_log(gid, "ROLE_DELETE", "AutoMod", f"Deleted role: {role.name}")
                                print(f"[AutoMod] Deleted role '{role.name}' from {guild.name}")
                                save_roles_for_guild(guild)

                    elif act == "edit_role_permissions":
                        role = guild.get_role(int(entry["role_id"]))
                        if not role:
                            print(f"[AutoMod] ⚠️  edit_role_permissions: role {entry['role_id']} not found")
                        elif role.managed:
                            print(f"[AutoMod] ⚠️  edit_role_permissions: '{role.name}' is managed — cannot edit")
                        else:
                            bot_top = max((r.position for r in guild.me.roles), default=0)
                            if role.position >= bot_top:
                                print(f"[AutoMod] ⚠️  edit_role_permissions: '{role.name}' is at/above bot's role — move bot's role higher in Server Settings")
                            else:
                                enabled = entry.get("permissions", [])
                                perms = discord.Permissions(**{name: True for name in enabled})
                                await role.edit(permissions=perms, reason="Dashboard permission edit")
                                add_log(gid, "ROLE_EDIT", "AutoMod", f"Updated permissions on: {role.name}")
                                print(f"[AutoMod] Updated permissions on '{role.name}' in {guild.name}: {enabled}")
                                save_roles_for_guild(guild)
                        ...
                        role   = guild.get_role(int(entry["role_id"]))
                        if member and role:
                            await member.add_roles(role, reason="Dashboard role assign")
                            add_log(gid, "ROLE_ASSIGN", member, f"Assigned role: {role.name}")
                            print(f"[AutoMod] Assigned '{role.name}' to {member} in {guild.name}")
                        ...
                        if member and role:
                            ...
                            add_log(gid, "ROLE_REMOVE", member, f"Removed role: {role.name}")
                            print(f"[AutoMod] Removed '{role.name}' from {member} in {guild.name}")

                except Exception as e:
                    print(f"[AutoMod] ⚠️  Action '{act}' failed in {guild.name}: {e}")
...
def save_roles_for_guild(guild: discord.Guild):
    roles = [
        {"id": str(r.id), "name": r.name,
         "color": str(r.colors.primary if hasattr(r, "colors") else r.color),
         "position": r.position, "managed": r.managed,
         "permissions": r.permissions.value}
        for r in sorted(guild.roles, key=lambda r: r.position, reverse=True)
        if not r.is_default()
    ]
    save_guild(guild.id, "roles.json", roles)

async def save_bans_for_guild(guild: discord.Guild):
    try:
        bans = []
        ...
        save_guild(guild.id, "bans.json", bans)
    except Exception:
        pass
...
async def on_guild_join(guild: discord.Guild):
    ...
    # Auto-detect who added the bot via audit logs and grant dashboard admin
    ...
    try:
        ...
        print(f"[AutoMod] ⚠️  Could not read audit logs for {guild.name}: {e}")

    if adder_id and adder_id != OWNER_ID:
        ...
        try:
            ...
            for uname, u in users.items():
                if u.get("discord_id") == adder_id and u.get("verified", True):
                    matched = uname
                    break
            if matched:
                if str(guild.id) not in users[matched].get("servers", []):
                    users[matched].setdefault("servers", []).append(str(guild.id))
                users[matched]["is_admin"] = True
                users[matched]["can_edit"] = True
                with open(users_path, "w") as f:
                    json.dump(users, f, indent=2)
                add_log(guild.id, "AUTO_ADMIN", matched, f"Auto-assigned dashboard admin via audit log")
                print(f"[AutoMod] ✅ Auto-assigned '{matched}' as dashboard admin of {guild.name}")
                try:
                    adder = await bot.fetch_user(int(adder_id))
                    await adder.send(
                        f"✅ **Dashboard access granted!**\n"
                        f"You added the bot to **{guild.name}** and have been automatically given "
                        f"admin access for that server in the dashboard.\n"
                        f"Log in to manage it.")
                except Exception:
                    pass
            else:
                print(f"[AutoMod] ⚠️  No verified dashboard account found for Discord ID {adder_id} in {guild.name}")
                print(f"[AutoMod]    Tip: Run /claim <code> in {guild.name} after signing up to link the server.")
        except Exception as e:
            print(f"[AutoMod] ⚠️  Auto-admin error for {guild.name}: {e}")
...
@bot.slash_command(name="claim", description="Claim dashboard admin access for this server using a code from the dashboard")
@discord.option("code", description="The claim code generated on the dashboard")
async def claim_cmd(ctx: discord.ApplicationContext, code: str):
    claims_path = os.path.join(DATA_DIR, "pending_claims.json")
    users_path  = os.path.join(DATA_DIR, "users.json")
    ...
    try:
        ...
        await ctx.respond("❌ No pending claim codes found.", ephemeral=True)
        return
    if code not in claims:
        await ctx.respond("❌ Invalid or expired code. Generate a new one from the dashboard.", ephemeral=True)
        return
    claim = claims[code]
    if str(ctx.author.id) != claim.get("discord_id"):
        await ctx.respond("❌ That code was generated for a different Discord account.", ephemeral=True)
        return
    try:
        with open(users_path) as f:
            users = json.load(f)
        ...
        if uname not in users:
            await ctx.respond("❌ Account not found — please sign up first.", ephemeral=True)
            return
        if str(ctx.guild.id) not in users[uname].get("servers", []):
            users[uname].setdefault("servers", []).append(str(ctx.guild.id))
        users[uname]["is_admin"] = True
        users[uname]["can_edit"] = True
        with open(users_path, "w") as f:
            json.dump(users, f, indent=2)
        del claims[code]
        with open(claims_path, "w") as f:
            json.dump(claims, f, indent=2)
        add_log(ctx.guild.id, "SERVER_CLAIMED", str(ctx.author), f"Dashboard admin claimed by '{uname}'")
        print(f"[AutoMod] ✅ '{uname}' claimed dashboard admin for {ctx.guild.name} via /claim")
        await ctx.respond(
            f"✅ **Server claimed!**\n"
            f"You now have dashboard admin access for **{ctx.guild.name}**.\n"
            f"Log in to the dashboard to start managing this server.", ephemeral=True)
    except Exception as e:
